...
for x in range(5):
    #Shortcut for creating a list containing five 0 elements and appending it to another list to make it a 3D list.
    board.append(["O"] * 5) 
...
#Where all the guessings and checkings happen
...
for turn in range(4):
    ...
    #if the user finds the row and col correctly.
    if guess_row == ship_row and guess_col == ship_col:
        print("Congratulations! You sunk my battleship!");
        break;

    #if its not right, then the measures taken to check whether its already guessed or its out of the board.
    else:
